...
- **Authentication**: 데모 역할 전환기를 노출하지 않는다. 로그인 계정에 부여된 역할 집합과 별도 기능 권한을 접근 기준으로 사용하고, 평가자·엔지니어 복합 계정에만 `사용 모드` 전환기를 제공한다. 실제 계정명·현재 사용 모드·로그아웃 행동을 데스크톱과 모바일에서 항상 확인할 수 있게 한다.
- **Evaluator insights capability**: 운영자가 `전체 현황·분석 열람` 기능 권한을 부여한 평가자는 평가자 모드에서 전체 현황과 분석 메뉴를 사용한다. 서버는 승인자와 동일한 집계 전용 투영만 반환하며 평가자 명단, 배정, 개별 평가지, 가중치, 감사 로그는 반환하지 않는다.
...
- **Fields**: 아이디, 표시 이름, 역할 구성, 역할별 연결 대상, 활성 상태. 평가자 역할에는 선택적 `전체 현황·분석 열람` 기능 권한을 제공한다. 단일 역할 외에 `평가자 · 엔지니어` 조합을 지원하며 이 경우 두 명단 연결을 모두 필수로 한다. 운영자·승인자는 연결값과 평가자 전용 기능 권한을 제거한다.
...
- **Parallel evaluator entry**: 평가자는 같은 시즌·같은 과제에 배정된 두 엔지니어를 선택해 하나의 통합 평가표에서 함께 입력한다. 문항 번호·평가항목·세부 기준은 한 번만 표시하고, 오른쪽에 발표자 A와 발표자 B의 점수 열을 나란히 둔다. 좁은 화면에서도 발표자 점수 열을 위아래 폼으로 분리하지 않으며 `문항 | A 점수 | B 점수` 3열 비교 구조를 유지한다. 두 발표자의 점수, 자동저장, 제출과 잠금 상태는 각 `assignmentId`에 독립적으로 귀속하며 한쪽 입력이 다른 쪽 초안을 덮어쓰지 않는다. 운영자 대리 입력 화면에는 이 모드를 노출하지 않는다.
- **Parallel selection**: 동시 평가를 시작할 때 과제를 먼저 고르고 좌·우 발표자를 각각 선택한다. 같은 발표자를 양쪽에 중복 선택할 수 없으며, 현재 평가자에게 동일 과제로 배정된 엔지니어가 두 명 이상일 때만 진입점을 표시한다.
- **Scheduled parallel presentation**: 운영자는 발표 일정 등록 시 동일 과제로 배정된 엔지니어 2명을 선택하고 `두 발표자를 한 화면에서 평가`를 활성화해 하나의 발표 묶음으로 저장한다. 평가자 달력과 오늘의 평가에서는 묶음을 한 건으로 표시하고, 일정에서 진입하면 공통 문항과 두 발표자 점수 열로 구성된 통합 평가표가 열린다. 우연히 날짜와 시간이 같은 별도 일정은 묶지 않는다.
