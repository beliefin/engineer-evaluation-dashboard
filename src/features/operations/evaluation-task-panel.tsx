...
export function EvaluationTaskPanel({ tasks, weightTotal, disabled, onSave, onDelete }: Props) {
  const seasonTotalIs100 = Math.abs(weightTotal - 100) < 0.000_001
  return (
    <OperationPanel
      aside={<div className="flex items-center gap-2"><Badge variant={seasonTotalIs100 ? "secondary" : "outline"}>합계 {weightTotal}%</Badge><EvaluationTaskDialog disabled={disabled} onSave={onSave} /></div>}
      description={<>과제별 평가방식, 세부 문항과 <span className="whitespace-nowrap">시즌 기본 가중치</span>를 설정합니다. 평가자는 과제에 고정하지 않고 엔지니어별 배정 탭에서 지정합니다.</>}
      title="시즌 과제 구성"
    >
      {tasks.length === 0 ? (
          ...
          {tasks.map((task) => (
            <article className="grid gap-4 p-4 lg:grid-cols-[minmax(0,1fr)_auto] lg:items-center" key={task.taskId}>
              <div className="min-w-0">
                ...
                <p className="mt-2 text-xs text-muted-foreground">{task.method === "evaluator_score" ? `평가 항목 ${task.items.length}개` : "P/F 또는 단일 결과 입력"} · {task.method.startsWith("evaluator") ? "엔지니어별 평가자 배정" : "운영자 입력"}</p>
              </div>
              <div className="flex items-center gap-2">
                <EvaluationTaskDialog disabled={disabled || task.locked} initial={task} onSave={onSave} />
