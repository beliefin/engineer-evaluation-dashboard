        ...
        <>
          ...
          <div className="grid divide-y lg:grid-cols-2 lg:divide-x lg:divide-y-0">
            <SourceColumn
              ...
              title="어학 성적"
            >
              {selected.languageNoScoreRecordId !== undefined ? (
                <NoScoreDeclaration
                  disabled={disabled}
                  label="보유 어학성적 없음 · 0점 반영"
                  onClear={() => onDeleteLanguageRecord(selected.languageNoScoreRecordId!)}
                />
            ...
            <SourceColumn
              ...
              title="자격증"
            >
              {selected.certificationNoScoreRecordId !== undefined ? (
                <NoScoreDeclaration
                  disabled={disabled}
                  label="보유 자격증 없음 · 0점 반영"
                  onClear={() => onDeleteCertificationRecord(selected.certificationNoScoreRecordId!)}
                />
                ...
                <EmptyRecord
                  action={(
                    <Button
                      disabled={disabled}
                      onClick={() => onSaveCertificationRecord({
                        recordId: null,
                        engineerId: selected.engineerId,
...
function NoScoreDeclaration({
  label,
  disabled,
  onClear,
}: Readonly<{ label: string; disabled: boolean; onClear: () => void }>) {
  return (
    <div className="flex items-center justify-between gap-3 rounded-md border border-success/30 bg-success-soft px-3 py-4">
      <div>
        ...
        <p className="mt-1 text-xs text-muted-foreground">미입력이 아닌 확정 0점으로 집계됩니다.</p>
      </div>
      <Button disabled={disabled} onClick={onClear} size="sm" variant="outline">
        선택 해제
      </Button>
    </div>
  )
