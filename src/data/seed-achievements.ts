...
const LANGUAGE_RESULTS = ["IH", "AL", "IM3", "IM2", "IM1"] as const
const CERTIFICATES = [
  ["산업안전기사", "기사", "한국산업인력공단"],
  ["전기기사", "기사", "한국산업인력공단"],
  ["설비보전기사", "기사", "한국산업인력공단"],
  ["컴퓨터활용능력 1급", "1급", "대한상공회의소"],
] as const
...
export function createSampleAchievementRecords(
  ...
  const languageScoreRecords = engineers.slice(0, 12).map((engineer, index) => ({
  ...
  const certificationRecords = engineers.slice(0, 12).map((engineer, index) => {
